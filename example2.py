...
# Globals to persist the browser and page
playwright_instance = None
browser_instance = None
page_instance = None

async def open_excalidraw_once():
    ...
    if playwright_instance is None:
        ...
        await page_instance.goto("https://excalidraw.com")
        await asyncio.sleep(3)  # wait for canvas to load
    return page_instance


@mcp.tool()
async def draw_rectangle_excalidraw(x1: int, y1: int, x2: int, y2: int) -> dict:
    try:
        page = await open_excalidraw_once()
        
        # 1. Bring to front and ensure focus
        await page.bring_to_front()
        
        # 2. Select the rectangle tool ('r')
        await page.keyboard.press("r")
        # Tiny delay to allow the tool to activate
        await asyncio.sleep(0.15)

        # 3. Move to start position
        await page.mouse.move(x1, y1)
        
        # 4. Mouse Down to start drawing
        await page.mouse.down()
        
        # 5. Manual interpolation loop (The "Magic" part)
        steps = 15
        for i in range(1, steps + 1):
            # Calculate the current intermediate point
            curr_x = x1 + (x2 - x1) * i / steps
            curr_y = y1 + (y2 - y1) * i / steps
            
            # Move mouse to the intermediate point
            await page.mouse.move(curr_x, curr_y)
            
            # Tiny sleep to let the canvas process the move event
            await asyncio.sleep(0.02)
            
        # 6. Release mouse to finish drawing
        await page.mouse.up()

        await page.keyboard.press("Enter")

        await page.keyboard.insert_text("dhruv")
        return {
            "content": [{"type": "text", "text": f"Rectangle drawn from ({x1},{y1}) to ({x2},{y2})"}]
        }
...
from gmail_client import get_gmail_service, create_message
    ...
    try:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"✅ Email sent successfully. Message ID: {sent['id']}"
                }
            ]
        }
